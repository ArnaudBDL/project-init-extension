...
This skill defines the mandatory engineering rules for Qwik work in this project.

## Role in This Project

Qwik is the selected resumable frontend framework. Its implementation must remain consistent with `00-META/context/stack.yml`, the active specification, architectural decisions and the existing codebase.

## Mandatory Rules

- Preserve resumability and lazy-loading boundaries.
- Use Qwik component and event APIs.
- Keep serializable state in resumable scopes.
- Use tasks only for explicit reactive work.
- Maintain server/client execution clarity.
...
- Do not introduce hydration-centric patterns.
- Do not capture non-serializable values in resumable closures.
- Do not move all logic into eagerly loaded modules.
